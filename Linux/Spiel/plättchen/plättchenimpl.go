...
func (p *data) Draw (resolution uint16,x,y uint16) { //Zeichnet die Plättchen
	...
	gfx.Volldreieck(dreieck1[0],dreieck1[1],dreieck1[2],dreieck1[3],dreieck1[4],dreieck1[5])		//Streifen von links unten nach recht oben wird gezeichnet
	gfx.Volldreieck(dreieck2[0],dreieck2[1],dreieck2[2],dreieck2[3],dreieck2[4],dreieck2[5])
	...
	gfx.Volldreieck(dreieck1[0],dreieck1[1],dreieck1[2],dreieck1[3],dreieck1[4],dreieck1[5])
	gfx.Volldreieck(dreieck2[0],dreieck2[1],dreieck2[2],dreieck2[3],dreieck2[4],dreieck2[5])
	...
	if l!=0 && o!=0 && r!=0 {										//Beschriftung der Plättchen findet nur statt, wenn es kein Nullerplättchen ist
		...
		gfx.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(30*resolution/10))
		var text string
		text = strconv.Itoa(int(l))									
		gfx.SchreibeFont(x-2*k/3,y+k/10,text)						// Lage der Zahl kommt es zu einem unterschiedlichen Versatz der Schrift zum Mittelpunkt
		text = strconv.Itoa(int(o))
		gfx.SchreibeFont(x-k/6,y-(k*433/500)+k/25,text)
		text = strconv.Itoa(int(r))									//für l,o und r müssen die Zahlen zuerst in einen string umgewandelt werden
		gfx.SchreibeFont(x+k/3,y+k/10,text)							//und dann im gfx-fenster angezeigt werden
	}
}
