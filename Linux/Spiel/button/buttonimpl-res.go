...
func (bt *data) Draw (resolution uint16) {		//Zeichenfunktion für Buttons		
	...
	gfx.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(20*resolution/10))	//Setzt den Font für den Text
	var text string
	text = (*bt).text										//Text wird zwischengeseichert in der Variablen "text"
	gfx.Stiftfarbe((*bt).GetTextFarbe())					//Textfarbe wird gesetzt
	...
	if i,ok:=enthältZeilenumbruch(text);!ok {	
		gfx.SchreibeFont(a,b,text)								//Schreibt den Font
	} else {
		var text1,text2 string
		text1 = text[:i]
		text2 = text[i+1:]
		gfx.SchreibeFont(a,b,text1)								//Schreibt den Font
		gfx.SchreibeFont(a,b+20*resolution/10,text2)
	}
}
