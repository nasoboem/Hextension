...
var (
  ...
  FONTPATH_DESC = "../fonts/LiberationSans-Regular.ttf"
  FONTPATH_FELD = "../fonts/LiberationMono-Bold.ttf"
  FONTHEIGHT    =  uint16(40)
  FONTWIDTH     =  uint16(25)
...
func New (x,y,anz int, ausrichtung byte, titel string) *feld {
  var f *feld = new (feld)
...
func (f *feld) SetzeTitel (nt string) {
	(*f).titel = nt
	refresh (f)
}
...
func (f *feld) Edit () string {
  ...
  // Eingabefeld hat den Focus, daher rot hinterlegt
//  (*f).r,(*f).g,(*f).b = 0xFF,0xCF,0xCF
  (*f).r,(*f).g,(*f).b = 0x9F,0x9F,0x9F
  refresh (f)
  gfx2.TastaturpufferAn ()

  Eingabe:
  for ;; {
    zeichen,gedrueckt,tiefe = gfx2.TastaturpufferLesen1 ()
  ...
  // Eingabefeld wieder als inaktiv markieren (=grau hinterlegt)
  (*f).r,(*f).g,(*f).b = R,G,B
  refresh (f)
  gfx2.TastaturpufferAus ()
...
func refresh (f *feld) {
  ...
  gfx2.Stiftfarbe ((*f).r,(*f).g,(*f).b)
  gfx2.Vollrechteck (uint16((*f).x-2),uint16((*f).y-2),uint16((*f).anz)*(*f).fontwd+4,(*f).fontht+4)
  // Texte werden immer in schwarz dargestellt
  gfx2.Stiftfarbe (0x0, 0x0, 0x0)
  // Titel in 12-Punkt setzen
  gfx2.SetzeFont (FONTPATH_DESC, 12)
  // Den Titel aber nur ausgeben, wenn er nicht leer ist.
  if len((*f).titel) > 0 {
    gfx2.SchreibeFont (uint16((*f).x), uint16((*f).y)+(*f).fontht+2, (*f).titel)
  }
  // Ggf. gespeicherten Text schreiben
  gfx2.SetzeFont ((*f).fontpfad, int((*f).fontht))
  utf8 = []rune ((*f).text)
  switch (*f).align {
    case 'r':
  ...
  gfx2.SchreibeFont (uint16((*f).x)+offset, uint16((*f).y), (*f).text)
}
