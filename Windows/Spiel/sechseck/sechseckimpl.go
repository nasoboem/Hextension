...
func (s *data) Draw () {					//Zeichnet das Sechseck durch die Angabe der 6 gleichseitigen Dreiecken aus denen es besteht
	...
	gfx2.Stiftfarbe (s.GibFarbe())			//Abrufen der Informationen
	x,y = s.GibKoordinaten()
	k = s.GibGroesse()
	h = k*433/500
	
	gfx2.Volldreieck(x,y,x-k,y,x-(k/2),y-h) //Das Sechseck besteht aus sechs gleichseitigen Dreiecken, die alle einzeln gezeichnet werden.
	gfx2.Volldreieck(x,y,x-k,y,x-(k/2),y+h)
	gfx2.Volldreieck(x,y,x+k,y,x+(k/2),y-(h))
	gfx2.Volldreieck(x,y,x+k,y,x+(k/2),y+(h))
	gfx2.Volldreieck(x,y,x+(k/2),y+(h),x-(k/2),y+(h))
	gfx2.Volldreieck(x,y,x+(k/2),y-(h),x-(k/2),y-(h))
}
