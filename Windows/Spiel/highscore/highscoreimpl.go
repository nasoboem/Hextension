...
func (h *data) Draw (resolution uint16,plaetze []int) {						//Zeichenfunktion für den Highscore; die plaetze eingetragenen Plazierungen werden rot hervorgehoben (neueintragungen in den Highscore), resolution passt die Ausgabe im gfx2.Fenster an
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))		//Festlegung von Font und Stiftfarbe
	gfx2.SchreibeFont(40*resolution,30*resolution,"Platz")					//Kopfzeile wird geschrieben
	gfx2.SchreibeFont(67*resolution,30*resolution,"Spieler")
	gfx2.SchreibeFont(100*resolution,30*resolution,"Punkte")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(3*resolution))		//Reduktion der Schriftgröße für die Einträge im Highscore
	for i:=uint16(0);i<uint16(len((*h).namen));i++ {						//Für die Länge der Namensliste
		if enthalten(i,plaetze) {											//Wenn der Index in der plaetze-Liste enthalten ist, dann
			gfx2.Stiftfarbe(255,0,0)											//dann nehme die Stiftfarbe rot
		} else {
			gfx2.Stiftfarbe(0,0,0)											//wenn nicht dann die Stiftfarbe schwarz
		}	
		gfx2.SchreibeFont(43*resolution,39*resolution+(7*resolution*i),fmt.Sprint(i+1)+".")	//Zeichnen der eigentlichen Einträge
		gfx2.SchreibeFont(70*resolution,39*resolution+(7*resolution*i),(*h).namen[i])
		gfx2.SchreibeFont(103*resolution,39*resolution+(7*resolution*i),fmt.Sprint((*h).punkte[i]))
	}
}	
