...
Anf:aktSeite = seite.SeitenAuswahl("Willkommen",seiten)													//Die erste Seite wird übergeben																
	...
	for aktSeite.GetName()!="GUI" && aktSeite.GetName()!="Beenden" && aktSeite.GetName()!="Konsole" {	//Schleife, die einen durch das Start-Menü führt - Aufruf der Seiten GUI, Konsole oder Beenden - beenden das Start-Menü 
		gfx2.UpdateAus()																					//Schaltet das Update aus, für ein flackerfreie darstellung der Seiten
		aktSeite.Draw(resolution)																		//Darstellung der Seiten
		seitenInhalt(resolution,aktSeite)																//Darstellung des Seiteninhalts 
		gfx2.UpdateAn()																					//Anschalten des Updates					
...
//Festlegung des Darstellungsmodus
	...
	if aktSeite.GetName() != "Beenden" {											//Wenn im Start-Menü nicht auf Beenden gedrückt wurde, dann wird das Spiel gestartet
		...
		if aktSeite.GetName() == "Konsole" {
			...
			gfx2.FensterAus()														//Ausschalten des Fensters der GUI
		}
...
//Konsole - Wechseln der Darstellungsform des Spiels - inKonsole wird auf true gesetzt, das Fenster geschlossen und an den Anfang der Konsolendarstellung des Spielfeldes gesprungen						
						...
						inKonsole = true
						gfx2.FensterAus()							//Fenster schließen
...
Menue2:					aktSeite = seite.SeitenAuswahl("Willkommen",seiten2)				//Aufruf der Startseite, des alternativen Startmenüs
						...
						if aktSeite.GetName()=="Beenden" {
							...
							gfx2.UpdateAus()													//Ablauf wie beim vorherigen Beenden nur mit anderem Ausstiegspunkt									
...
//Darstellung der Spielergebnisse im GUI
			...
			gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(6*resolution))
			gfx2.Stiftfarbe(0,0,0)
			gfx2.SchreibeFont(60*resolution,18*resolution,"Endstand")					//Überschrift
			gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))
			gfx2.SchreibeFont(40*resolution,30*resolution,"Platz")						//Tabellenüberschriften
			gfx2.SchreibeFont(67*resolution,30*resolution,"Spieler")
			gfx2.SchreibeFont(100*resolution,30*resolution,"Punkte")
			gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(3*resolution))
			for i:=uint16(0);i<uint16(len(spielernamen));i++ {							//Inhalt der Tabelle wird geschrieben
				gfx2.SchreibeFont(43*resolution,39*resolution+(7*resolution*i),fmt.Sprint(i+1)+".")
				gfx2.SchreibeFont(70*resolution,39*resolution+(7*resolution*i),spielernamen[i])
				gfx2.SchreibeFont(103*resolution,39*resolution+(7*resolution*i),fmt.Sprint(punkte[i]))
			}
			gfx2.UpdateAn()
...
//Darstellung des Highscores
			...
			gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(6*resolution))
			gfx2.Stiftfarbe(0,0,0)
			gfx2.SchreibeFont(59*resolution,18*resolution,"Highscore")					//Überschrift
...
func spielereingabe (resolution uint16) []string { //Spielereingabe für die GUI - stammt noch aus Zeiten vor ADT-Seite, daher nicht mit Seite implementiert
	gfx2.UpdateAus()
	gfx2.Stiftfarbe(255,255,255)						//Hintergrund wird weiß gemacht
	gfx2.Cls()
	drawHintergrund(resolution)						//Plättchenrand wird gezeichnet
	gfx2.UpdateAn()
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution)) //Texte mit Anweisungen an den Spieler werden dargestellt
	gfx2.SchreibeFont(25*resolution,20*resolution,"Wählen Sie die Anzahl der Spieler aus (max. 6)")	
	gfx2.SchreibeFont(35*resolution,275*resolution/10,"und geben Sie sie nacheinander ein.")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))
	gfx2.SchreibeFont(598*resolution/10,45*resolution,"Anzahl der Spieler")
	...
	f.SetzeZeichensatzgroesse (groessenauswahl(resolution))					//Zum einstellen der Schriftgröße wurde eine eigene Funktion geschrieben, da Herr Schäfer seine eigenen Standardgrößen definiert und nur indirekt auf gfx2 zurückgreift
	...
	gfx2.Archivieren()														//Das gfx2-Fenster wird gespeicher, damit es für jede Spielereingabe neu restauriert werden kann 
	...
	for i:=0;i<anzahlSpieler;i++ {											//Eingabe wird so oft wiederholt, bis alle Spielernamen eingegeben sind
		gfx2.Stiftfarbe(0,0,0)
		gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))
		gfx2.SchreibeFont(598*resolution/10,625*resolution/10,"Name von Spieler " + fmt.Sprint(i+1) ) //fmt.Sprint wandelt i+1 in ein String um
		...
		if !(i==(anzahlSpieler-1)) {										//Wenn es nicht der letzte Spieler ist, dann
			...
			gfx2.Cls()														//das ganze Fenster weiß gemacht
			gfx2.Restaurieren(1,1,160*resolution,100*resolution)				//das ganze Fenster resauriert und es beginnt die nächste Spielernameneingabe
			gfx2.UpdateAn()
		}
	}
...
func aboutText (resolution uint16) { //Liefert den den formatierten Text, der auf der About-Seite ausgegeben wird
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(6*resolution))
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SchreibeFont(20*resolution,15*resolution,"Hextension")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(20*resolution,24*resolution,"Das Spiel")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(20*resolution,28*resolution,"Hextension ist ein Brettspiel für eine prinzipiell unbegrenzte Anzahl von Spie-")
	gfx2.SchreibeFont(20*resolution,305*resolution/10,"lern. Es wurde 1983 unter dem Namen Hextension von Peter Burley entwickelt und von")
	gfx2.SchreibeFont(20*resolution,33*resolution,"Spear-Spiele vertrieben. Die Grafik der Originalversion gestaltete Franz Vohwinkel.")
	gfx2.SchreibeFont(20*resolution,355*resolution/10,"Unter dem Namen Take-it-easy wird das Spiel seit 1994 vom Verlag F.X. Schmid, jetzt")
	gfx2.SchreibeFont(20*resolution,38*resolution,"Ravensburger, vertrieben.")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(20*resolution,44*resolution,"Das Programm")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(20*resolution,48*resolution,"Das vorliegende Programm wurde von Jakob Hättig im Rahmen der Lehrerweiterbildung")
	gfx2.SchreibeFont(20*resolution,505*resolution/10,"entwickelt. Der unbedarfte Programmierer sei gewarnt. Man sagt, dass selbst der")
	gfx2.SchreibeFont(20*resolution,53*resolution,"Blick in die Implementierung einens einfachen ADTs, einen den Verstand kosten kann.")
	gfx2.SchreibeFont(20*resolution,555*resolution/10,"Wer es wagen sollte gar Hand an den Quellcode zu legen, der muss damit rechnen")
	gfx2.SchreibeFont(20*resolution,58*resolution,"so zu enden wie der Autor.")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(20*resolution,63*resolution,"Der Programmierer")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(20*resolution,67*resolution,"Jakob Hättig, so sagt man, war ein einfacher Lehrer für Chemie und Biologie,")
	gfx2.SchreibeFont(20*resolution,695*resolution/10,"bis er anfing in Go Hextension zu programmieren. Am Ende wiederholte er")
	gfx2.SchreibeFont(20*resolution,72*resolution,"immer nur diesen einen Satz:")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(35*resolution/10))
	gfx2.SchreibeFont(20*resolution,755*resolution/10,"\"Haben Sie es ausprobiert!\"")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(20*resolution,80*resolution,"Dann verschwand er eines Tages und ward nicht mehr gesehen.")
}

func drawBegrüßungsText (resolution uint16) { //Liefert die Inhalte der Startseite
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SchreibeFont(638*resolution/10,42*resolution,"      by      ")
	gfx2.SchreibeFont(638*resolution/10,485*resolution/10," Jakob Hättig ")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(10*resolution))
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SchreibeFont(502*resolution/10,25*resolution,"Hextension")
...
func highscoreSeite (resolution uint16) {		//Liefert den Inhalt der Highscore-Seite
	...
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(6*resolution))
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SchreibeFont(59*resolution,18*resolution,"Highscore")
...
func beenden2 (resolution uint16) { 	//Inhalt der "Wirklich Beenden"-Seite
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(4*resolution))
	gfx2.SchreibeFont(305*resolution/10,450*resolution/10,"Wollen Sie wirklich dieses Spiel beenden?")
}
...
func regeln1Text (resolution uint16) { //Inhalt des Textfeldes zur Regel-Seite 1
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,157*resolution/10,"Das Spielfeld")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,199*resolution/10,"Es besteht aus einer sechs-")
	gfx2.SchreibeFont(1147*resolution/10,224*resolution/10,"eckigen Legefläche, mit 19")
	gfx2.SchreibeFont(1147*resolution/10,249*resolution/10,"Feldern, Anzeigen für Zug,")
	gfx2.SchreibeFont(1147*resolution/10,274*resolution/10,"Punktestand und Name des")
	gfx2.SchreibeFont(1147*resolution/10,299*resolution/10,"Spielers, dessen Spielfeld")
	gfx2.SchreibeFont(1147*resolution/10,324*resolution/10,"gerade angezeigt wird.")
	gfx2.SchreibeFont(1147*resolution/10,359*resolution/10,"Das aktuelle Plättchen,")
	gfx2.SchreibeFont(1147*resolution/10,384*resolution/10,"zeigt das bei diesem Zug")
	gfx2.SchreibeFont(1147*resolution/10,409*resolution/10,"zu spielende Plättchen an.")
	gfx2.SchreibeFont(1147*resolution/10,434*resolution/10,"Zur Zeit ist es noch leer.")
	gfx2.SchreibeFont(1147*resolution/10,469*resolution/10,"Ganz unten sind die 27 noch")
	gfx2.SchreibeFont(1147*resolution/10,494*resolution/10,"im Spiel befindlichen ")
	gfx2.SchreibeFont(1147*resolution/10,519*resolution/10,"Plättchen zu sehen. Zur Zeit")
	gfx2.SchreibeFont(1147*resolution/10,544*resolution/10,"sind das noch alle Plättchen.")
	gfx2.SchreibeFont(1530*resolution/10,768*resolution/10,"1/4")
}
...
func regeln2Text (resolution uint16) {	//Inhalt des Textfeldes für die Regel-Seite 2
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,157*resolution/10,"Der Zug")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,199*resolution/10,"Von den Plättchen im Spiel")
	gfx2.SchreibeFont(1147*resolution/10,224*resolution/10,"wird zufällig eines aus-")
	gfx2.SchreibeFont(1147*resolution/10,249*resolution/10,"gewählt, welches dann zum ")
	gfx2.SchreibeFont(1147*resolution/10,274*resolution/10,"aktuellen Plättchen wird.")
	gfx2.SchreibeFont(1147*resolution/10,309*resolution/10,"Der Spieler muss, durch")
	gfx2.SchreibeFont(1147*resolution/10,334*resolution/10,"anklicken eines Feldes")
	gfx2.SchreibeFont(1147*resolution/10,359*resolution/10,"der Legefläche, auswählen")
	gfx2.SchreibeFont(1147*resolution/10,384*resolution/10,"wo er das Plättchen hin-")
	gfx2.SchreibeFont(1147*resolution/10,409*resolution/10,"legen möchte.")
	gfx2.SchreibeFont(1530*resolution/10,768*resolution/10,"2/4")
}
...
func regeln3Text (resolution uint16) {		//Inhalt des Textfeldes für die Regel-Seite 3
	...
	gfx2.SchreibeFont(1147*resolution/10,157*resolution/10,"Der Zug")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,199*resolution/10,"Der Spieler kann das")
	gfx2.SchreibeFont(1147*resolution/10,224*resolution/10,"Plättchen solange an andere")
	gfx2.SchreibeFont(1147*resolution/10,249*resolution/10,"Positionen auf der Lege-")
	gfx2.SchreibeFont(1147*resolution/10,274*resolution/10,"fläche bewegen, bis er den")
	gfx2.SchreibeFont(1147*resolution/10,299*resolution/10,"Weiter-Knopf anklickt.")
	gfx2.SchreibeFont(1147*resolution/10,334*resolution/10,"Damit beendet der Spieler")
	gfx2.SchreibeFont(1147*resolution/10,359*resolution/10,"seinen Zug.")
	gfx2.SchreibeFont(1147*resolution/10,394*resolution/10,"Bei mehreren Spielern ist")
	gfx2.SchreibeFont(1147*resolution/10,419*resolution/10,"jetzt entweder der nächste")
	gfx2.SchreibeFont(1147*resolution/10,444*resolution/10,"Spieler dran, der auch das")
	gfx2.SchreibeFont(1147*resolution/10,469*resolution/10,"Plättchen auf seiner Lege-")
	gfx2.SchreibeFont(1147*resolution/10,494*resolution/10,"fläche plazieren muss, oder")
	gfx2.SchreibeFont(1147*resolution/10,519*resolution/10,"es gibt eine neue Zugrunde")
	gfx2.SchreibeFont(1147*resolution/10,544*resolution/10,"mit einem neuen aktuellen")
	gfx2.SchreibeFont(1147*resolution/10,569*resolution/10,"Plättchen.")
	gfx2.SchreibeFont(1147*resolution/10,604*resolution/10,"Dies wiederholt sich dann")
	gfx2.SchreibeFont(1147*resolution/10,629*resolution/10,"so lange, bis die gesamte")
	gfx2.SchreibeFont(1147*resolution/10,654*resolution/10,"Legefläche voll ist.")
	gfx2.SchreibeFont(1147*resolution/10,689*resolution/10,"Es werden also nur 19 ")
	gfx2.SchreibeFont(1147*resolution/10,714*resolution/10,"Plättchen von den ")
	gfx2.SchreibeFont(1147*resolution/10,739*resolution/10,"27 möglichen gezogen.")
	gfx2.SchreibeFont(1530*resolution/10,768*resolution/10,"3/4")
}
...
func regeln4Text (resolution uint16) {	//Inhalt des Textfeldes für die Regel-Seite 4
	gfx2.Stiftfarbe(0,0,0)
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(32*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,157*resolution/10,"Ziel des Spiels")
	gfx2.SetzeFont("../fonts/LiberationMono-Bold.ttf",int(25*resolution/10))
	gfx2.SchreibeFont(1147*resolution/10,199*resolution/10,"Ziel des Spiels ist es")
	gfx2.SchreibeFont(1147*resolution/10,224*resolution/10,"möglichst viele Punkte zu ")
	gfx2.SchreibeFont(1147*resolution/10,249*resolution/10,"bekommen.")
	gfx2.SchreibeFont(1147*resolution/10,284*resolution/10,"Punkte gibt es aber nur,")
	gfx2.SchreibeFont(1147*resolution/10,309*resolution/10,"für vollständige, ununter-")
	gfx2.SchreibeFont(1147*resolution/10,334*resolution/10,"brochene Reihen in der")
	gfx2.SchreibeFont(1147*resolution/10,359*resolution/10,"selben Zahl und Farbe.")
	gfx2.SchreibeFont(1147*resolution/10,394*resolution/10,"Dabei berechnen sich die")
	gfx2.SchreibeFont(1147*resolution/10,419*resolution/10,"Punkte nach der Anzahl der")
	gfx2.SchreibeFont(1147*resolution/10,444*resolution/10,"Plättchen mal deren Zahl.")
	gfx2.SchreibeFont(1147*resolution/10,479*resolution/10,"Daher ergeben sich für die")
	gfx2.SchreibeFont(1147*resolution/10,504*resolution/10,"1er-Reihe 3 Punkte und für")
	gfx2.SchreibeFont(1147*resolution/10,529*resolution/10,"die 9er-Reihe 45 Punkte.")
	gfx2.SchreibeFont(1147*resolution/10,554*resolution/10,"Die Diagonale bringt keine")
	gfx2.SchreibeFont(1147*resolution/10,579*resolution/10,"Punkte, da hier verschiedene")
	gfx2.SchreibeFont(1147*resolution/10,604*resolution/10,"Zahlen in einer Reihe liegen.")
	gfx2.SchreibeFont(1147*resolution/10,624*resolution/10,"")
	gfx2.SchreibeFont(1147*resolution/10,649*resolution/10,"")
	gfx2.SchreibeFont(1147*resolution/10,674*resolution/10,"Viel Spaß beim spielen!")
	gfx2.SchreibeFont(1530*resolution/10,768*resolution/10,"4/4")
}


func textFeldDraw (resolution uint16) {	//Textfeld Hintergrund für die Regel-Seiten
	gfx2.Stiftfarbe(0,0,0)
	gfx2.Vollrechteck(114*resolution,15*resolution,44*resolution,65*resolution)
	gfx2.Stiftfarbe(255,255,255)
	gfx2.Vollrechteck(1142*resolution/10,152*resolution/10,436*resolution/10,646*resolution/10)
}
